...
## 12. Phased Build Sequence & Roadmap (Modular Component + Settings Architecture)

```
[Phase 0: Log Keeper Diagnostic Engine & Deep-Link Intent Routing]
               │  (Active from Day 1, reachable directly from Comma 🪵 shortcut)
               ▼
[Phase 1: Minimal IME Service, Onboarding & Settings Navigation Hub]
               │  (MainActivity Composable Navigation connecting all sub-settings)
               ▼
[Phase 2: Touch Dispatcher, Canvas Key Matrix & Key Visual Hints]
               │  (Top-right symbol hints, rounded pill keycaps, Spacebar glide, Swipe delete)
               │  + Layout & Appearance Settings Sub-Page
               ▼
[Phase 3: Suggestion Strip, Autocorrect & Binary Dictionary Engine]
               │  (3-slot predictions, clipboard paste pills, mmap zero-heap .dict loader)
               │  + Dictionary & Prediction Settings Sub-Page
               ▼
[Phase 4: DesktopKeyboardView & Zero-Search Modals]
               │  (3-Row Fat-Key Nav Pad, Emoji, Clipboard, Prompt List)
               │  + Custom Shortcuts & Gestures Settings Sub-Page
               ▼
[Phase 5: Security Vaults (KeePass .kdbx, 2FA/TOTP, Privacy Pills)]
               │  (Zero-clipboard injection, in-keyboard 9-dot pattern unlock)
               │  + Security Vault & Personal Vault Settings Sub-Pages
               ▼
[Phase 6: Offline Neural Voice Input & Modular ZIP Backup/Restore Engine]
               │  (Whisper/Silero VAD listening sheet + HeliBoard & VianBoard ZIP backup importer)
               │  + Voice Models & Backup Settings Sub-Pages
```
